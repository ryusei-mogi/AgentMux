...
AgentMux は OpenCode GO 専用のマルチアカウント管理ツールです。OpenCode を `http://127.0.0.1:8787/v1` に向けるだけで、AgentMux が複数の OpenCode GO サブスクリプションアカウントを自動的に切り替えます。
...
- 複数の OpenCode GO アカウントを、ひとつのローカルエンドポイントで管理する。
...
model:    deepseek-v4-flash など AgentMux の論理モデル名
```
...
クライアントは `deepseek-v4-flash` を指定します。AgentMux はその論理モデルに紐づく upstream の中から実際の送信先を選びます。

### Upstream

upstream は OpenCode GO アカウントのエンドポイント、認証情報、モデル名の対応をまとめたものです。AgentMux がサポートする upstream の type は `openai-compatible` のみです。OpenCode GO アカウントの場合、`base_url` は常に `https://opencode.ai/zen/go/v1` です。
...
- `deepseek-v4-flash` のような key が、クライアントから指定する論理モデル名です。
...
- `type`: 常に `openai-compatible` です。これが唯一のサポート対象 type です。
- `base_url`: OpenCode GO の base URL である `https://opencode.ai/zen/go/v1` を指定します。
- `api_key_env`: upstream の API キーを入れた環境変数名です。
- `api_key`: upstream の API キーを直接書く項目です。通常は `api_key_env` を使います。
...
デフォルト戦略です。複数の OpenCode GO アカウントを運用する構成に向いています。
...
利用可能な provider preset を一覧表示します。
...
- `opencode-go` — OpenCode GO アカウントの設定テンプレート

preset は upstream 設定の断片です。実際に使うには `id`、`models`、認証情報を追加してください。

## よく使う構成例

### 3つの OpenCode GO アカウントを使う
