...
export async function proxyChatCompletion(
  ...
  for (const candidate of candidates.slice(0, maxAttempts)) {
    ...
    try {
      ...
      const upstreamResponse = await callUpstream(config, candidate, upstreamBody);
      ...
      if (body.stream === true) {
        ...
        return {
          response: openAIStreamResponse(upstreamResponse, {
            requestId,
            started,
            model: body.model,
            fallbackInput,
      ...
      const usage = usageFromResponse(upstreamJson, fallbackInput);
      store.recordUsage({
        request_id: requestId,
        model: body.model,
        upstream_id: candidate.upstream.id,
      ...
      store.recordSuccess(candidate.upstream.id);
      return {
        response: Response.json(upstreamJson, { status: 200 }),
        upstreamId: candidate.upstream.id
      };
    } catch (error) {
      const reason =
...
async function callUpstream(
  ...
  try {
    const url = `${candidate.upstream.base_url.replace(/\/$/, '')}/chat/completions`;
...
function requestHeaders(candidate: Candidate): Headers {
